...
SHEN_SHA_INFO = {
    ...
    '紅豔煞': {'feature': '浪漫、多情、色慾、藝術感。', 'effect': '增加魅力，但需防感情生活混亂。'},
...
def get_55_shen_sha(bazi, pillar_idx):
    ...
    if (pillar_idx != 0 and t_b == hg_map.get(y_b)) or (pillar_idx != 2 and t_b == hg_map.get(d_b)):
        found.append("華蓋")
...
def render_chart(bazi):
    ...
    pillar_data = [{"title":"年柱","p":bazi.pillars[0],"idx":0},{"title":"月柱","p":bazi.pillars[1],"idx":1},{"title":"日柱","p":bazi.pillars[2],"idx":2},{"title":"時柱","p":bazi.pillars[3],"idx":3}]
    results = []
    all_found_shen_sha = set()
    
    for p in pillar_data:
        h = HIDDEN_STEMS_DATA.get(bazi.branches[p["idx"]], [])
        s_sha = get_55_shen_sha(bazi, p["idx"])
        all_found_shen_sha.update(s_sha)
        results.append({
            "title":p["title"], "ten_god": get_ten_god(me_stem, bazi.stems[p["idx"]]) if p["title"] != "日柱" else "日主",
            "stem":bazi.stems[p["idx"]], "branch":bazi.branches[p["idx"]], "nayin":NAYIN_DATA.get(p["p"], ""),
    ...
    rel_html = f"""<div style="margin-top: 35px; font-family: '標楷體'; text-align: left; padding: 25px; border: 2.5px solid #2c3e50; border-radius: 15px; background: #ffffff;">
        <h2 style="color: #2c3e50; text-align: center; border-bottom: 2px solid #2c3e50;">📜 四柱干支交互關係詳解</h2>
    ...
    # --- 新增：神煞深度解析區塊 (對應 Excel) ---
    shen_sha_detail_list = []
    for ss in sorted(list(all_found_shen_sha)):
        info = SHEN_SHA_INFO.get(ss, {'feature': '暫無資料', 'effect': '暫無資料'})
        shen_sha_detail_list.append(f"""
            <tr>
                <td style="border: 1px solid #ccc; padding: 10px; font-weight: bold; color: #8e44ad; width: 120px;">{ss}</td>
                <td style="border: 1px solid #ccc; padding: 10px; color: #2c3e50; text-align: left;">{info['feature']}</td>
                <td style="border: 1px solid #ccc; padding: 10px; color: #d35400; text-align: left;">{info['effect']}</td>
            </tr>
        """)
    ...
    ss_html = f"""<div style="margin-top: 35px; font-family: '標楷體'; text-align: center; padding: 25px; border: 2.5px solid #8e44ad; border-radius: 15px; background: #fdfbff;">
        ...
        <table style="width:100%; border-collapse: collapse; margin-top: 15px;">
            ...
            {"".join(shen_sha_detail_list) if shen_sha_detail_list else "<tr><td colspan='3' style='padding:20px;'>本命盤無顯著特殊神煞</td></tr>"}
        </table>
    </div>"""
...
col_d, col_g = st.columns([3, 1])
with col_d: birth_date = st.date_input("選擇日期", value=datetime.date(1990, 1, 1), min_value=datetime.date(1900, 1, 1))
with col_g: gender = st.radio("性別", ["男", "女"], horizontal=True)
