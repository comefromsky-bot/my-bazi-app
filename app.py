...
def get_shen_sha_per_pillar(bazi, pillar_idx):
    ...
    # 天乙 (依據您提供的數據: 巳,未,申,酉,亥,丑,子,丑,寅,卯)
    tian_yi_year = {'甲':'巳', '乙':'未', '丙':'申', '丁':'酉', '戊':'亥', '己':'丑', '庚':'子', '辛':'丑', '壬':'寅', '癸':'卯'}
    if branch == tian_yi_year.get(me): found.append("天乙")

    # 玉堂 (依據: 卯,丑,子,亥,酉,未,申,未,午,巳)
    yu_tang_year = {'甲':'卯', '乙':'丑', '丙':'子', '丁':'亥', '戊':'酉', '己':'未', '庚':'申', '辛':'未', '壬':'午', '癸':'巳'}
    if branch == yu_tang_year.get(me): found.append("玉堂")

    # 文昌 (依據: 卯,巳,午,申,酉,申,酉,亥,子,寅)
    wen_chang_year = {'甲':'卯', '乙':'巳', '丙':'午', '丁':'申', '戊':'酉', '己':'申', '庚':'酉', '辛':'亥', '壬':'子', '癸':'寅'}
    if branch == wen_chang_year.get(me): found.append("文昌")

    # 真文昌 (柱位對照)
    zhen_wen_chang = {'甲':'乙卯', '乙':'己巳', '丙':'壬午', '丁':'丙申', '戊':'己酉', '己':'庚申', '庚':'癸酉', '辛':'丁亥', '壬':'庚子', '癸':'壬寅'}
    if pillar == zhen_wen_chang.get(me): found.append("真文昌")

    # 元祿神
    yuan_lu = {'甲':'子', '乙':'寅', '丙':'卯', '丁':'巳', '戊':'午', '己':'巳', '庚':'午', '辛':'申', '壬':'酉', '癸':'亥'}
    if branch == yuan_lu.get(me): found.append("元祿神")

    # 元星
    yuan_xing = {'甲':'申', '乙':'寅', '丙':'亥', '丁':'卯', '戊':'戌', '己':'丑', '庚':'子', '辛':'辰', '壬':'酉', '癸':'巳'}
    if branch == yuan_xing.get(me): found.append("元星")

    # 天廚
    tian_chu = {'甲':'亥', '乙':'巳', '丙':'午', '丁':'子', '戊':'巳', '己':'午', '庚':'申', '辛':'寅', '壬':'午', '癸':'酉'}
    if branch == tian_chu.get(me): found.append("天廚")

    # 太極貴人 (列表判斷)
    taiji = {
    '甲':['申','巳'], '乙':['子','午'], '丙':['子','午'], '丁':['卯','酉'], '戊':['卯','酉'], 
    '己':['辰','戌','丑','未'], '庚':['辰','戌','丑','未'], '辛':['寅','亥'], '壬':['寅','亥'], '癸':['申','巳']
    }
    if branch in taiji.get(me, []): found.append("太極貴人")

    # 文昌貴
    wen_chang_gui = {'甲':'丑', '乙':'巳', '丙':'亥', '丁':'戌', '戊':'辰', '己':'申', '庚':'午', '辛':'寅', '壬':'未', '癸':'卯'}
    if branch == wen_chang_gui.get(me): found.append("文昌貴")

    # 文星貴
    wen_xing_gui = {'甲':'卯', '乙':'午', '丙':'巳', '丁':'申', '戊':'酉', '己':'申', '庚':'酉', '辛':'戌', '壬':'亥', '癸':'寅'} 
    if branch == wen_xing_gui.get(me): found.append("文星貴")

    # 天印貴
    tian_yin_gui = {'甲':'卯', '乙':'寅', '丙':'亥', '丁':'戌', '戊':'酉', '己':'申', '庚':'未', '辛':'午', '壬':'巳', '癸':'辰'}
    if branch == tian_yin_gui.get(me): found.append("天印貴")

    # 天福
    tian_fu = {'甲':'巳', '乙':'酉', '丙':'申', '丁':'子', '戊':'亥', '己':'卯', '庚':'寅', '辛':'午', '壬':'巳', '癸':'午'}
    if branch == tian_fu.get(me): found.append("天福")

    # 天官
    tian_guan = {'甲':'午', '乙':'未', '丙':'辰', '丁':'巳', '戊':'寅', '己':'卯', '庚':'酉', '辛':'亥', '壬':'酉', '癸':'戌'}
    if branch == tian_guan.get(me): found.append("天官")

    # 歲德 (天干對照)
    sui_de = {'甲':'戊', '乙':'甲', '丙':'庚', '丁':'丙', '戊':'壬', '己':'甲', '庚':'甲', '辛':'庚', '壬':'丙', '癸':'壬'}
    if stem == sui_de.get(me): found.append("歲德")

    # 歲德合 (天干對照)
    sui_de_he = {'甲':'癸', '乙':'己', '丙':'乙', '丁':'辛', '戊':'丁', '己':'己', '庚':'己', '辛':'乙', '壬':'辛', '癸':'丁'}
    if stem == sui_de_he.get(me): found.append("歲德合")

    # 歲干合 (天干對照)
    sui_gan_he = {'甲':'戊', '乙':'己', '丙':'庚', '丁':'辛', '戊':'壬', '己':'癸', '庚':'甲', '辛':'乙', '壬':'丙', '癸':'丁'}
    if stem == sui_gan_he.get(me): found.append("歲干合")
 
    # 真陽貴 (柱位)
    zhen_yang_gui = {'甲':'丁巳', '乙':'辛未', '丙':'甲申', '丁':'丁酉', '戊':'辛亥', '己':'乙丑', '庚':'丙子', '辛':'己丑', '壬':'庚寅', '癸':'癸卯'}
    if pillar == zhen_yang_gui.get(me): found.append("真陽貴")

    # 真陰貴 (柱位)
    zhen_yin_gui = {'甲':'乙卯', '乙':'丁丑', '丙':'戊子', '丁':'乙亥', '戊':'己酉', '己':'己未', '庚':'壬申', '辛':'癸未', '壬':'甲午', '癸':'乙巳'}
    if pillar == zhen_yin_gui.get(me): found.append("真陰貴")

    # 真命祿 (柱位)
    zhen_ming_lu = {'甲':'甲子', '乙':'丙寅', '丙':'己卯', '丁':'癸巳', '戊':'丙午', '己':'丁巳', '庚':'庚午', '辛':'甲申', '壬':'丁酉', '癸':'辛亥'}
    if pillar == zhen_ming_lu.get(me): found.append("真命祿")

    # 武曲星
    wu_qu = {'甲':'酉', '乙':'亥', '丙':'子', '丁':'寅', '戊':'卯', '己':'寅', '庚':'卯', '辛':'巳', '壬':'辰', '癸':'申'}
    if branch == wu_qu.get(me): found.append("武曲星")

    # 金神七煞 (多個地支判斷)
    jin_shen_7_sha = {
    '甲':['申','酉'], '乙':['未'], '丙':['辰','巳'], '丁':['子','丑','寅','卯'], '戊':['戌','亥'], 
    '己':['申','酉'], '庚':['午','未'], '辛':['辰','巳'], '壬':['子','丑','寅','卯'], '癸':['戌','亥']
    }
    if branch in jin_shen_7_sha.get(me, []): found.append("金神七煞")

    # 紅艷煞 (通常查時支)
    hong_yan_year = {'甲':'申', '乙':'午', '丙':'申', '丁':'寅', '戊':'未', '己':'辰', '庚':'辰', '辛':'戌', '壬':'酉', '癸':'子'}
    if branch == hong_yan_year.get(me): found.append("紅艷煞")

    # 流霞 (通常查日時支)
    liu_xia_year = {'甲':'寅', '乙':'酉', '丙':'戌', '丁':'未', '戊':'申', '己':'巳', '庚':'午', '辛':'辰', '壬':'卯', '癸':'亥'}
    if branch == liu_xia_year.get(me): found.append("流霞")

    # 天掃星 (柱位)
    tian_sao = {'甲':'甲戌', '乙':'癸未', '丙':'壬午', '丁':'辛巳', '戊':'庚辰', '己':'卯', '庚':'戊寅', '辛':'丁丑', '壬':'丙子', '癸':'乙亥'}
    if pillar == tian_sao.get(me): found.append("天掃星")

    # 節度貴人
    jie_du = {'甲':'丑', '乙':'巳', '丙':'未', '丁':'巳', '戊':'未', '己':'巳', '庚':'未', '辛':'亥', '壬':'丑', '癸':'亥'}
    if branch == jie_du.get(me): found.append("節度貴人")

    # 金輿
    jin_yu_year = {'甲':'寅', '乙':'辰', '丙':'巳', '丁':'未', '戊':'申', '己':'未', '庚':'申', '辛':'戌', '壬':'亥', '癸':'丑'}
    if branch == jin_yu_year.get(me): found.append("金輿")

    # 福星貴人 (依據數據特殊組合判斷)
    fu_xing_year = {
    '甲':['乙卯','乙丑'], '乙':['丙寅','丙子'], '丙':['丁亥','丁丑'], '丁':['戊戌'], 
    '戊':['己酉'], '己':['庚申'], '庚':['辛未'], '辛':['壬辰'], '壬':['癸巳'], '癸':['甲辰']
    }
    if pillar in fu_xing_year.get(me, []): found.append("福星貴人")
